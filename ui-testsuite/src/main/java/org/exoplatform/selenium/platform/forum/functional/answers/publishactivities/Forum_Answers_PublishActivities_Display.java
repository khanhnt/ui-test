...
public class Forum_Answers_PublishActivities_Display extends AnswerBase{
	...
	public void test06_NotDisplayAnswerActivityAfterApproveOrDisapproveQuestion() {
		...
		checkActivityOfQuestion(questName);
	...
	//This function for case test06
	public void checkActivityOfQuestion(String questName){

		navTool.goToHomePage();
		waitForAndGetElement(By.linkText(questName));
		waitForElementNotPresent(hpAct.ELEMENT_COMMENT_BLOCK.replace("${title}", questName));
		waitForAndGetElement(hpAct.ELEMENT_QUESTION_CONTENT.replace("${title}", questName));
		waitForAndGetElement(hpAct.ELEMENT_QUESTION_NUM_COMMENT.replace("${title}", questName).replace("${number}", "No"));
	}
}
